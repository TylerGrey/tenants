...
func Ping(dbName string) error {
	mysqlMasterConnStr := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", os.Getenv("MYSQL_USER"), os.Getenv("MYSQL_PASSWORD"), os.Getenv("MYSQL_MASTER_HOST"), os.Getenv("MYSQL_PORT"), dbName)
	...
	mysqlReplicaConnStr := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", os.Getenv("MYSQL_USER"), os.Getenv("MYSQL_PASSWORD"), os.Getenv("MYSQL_REPLICA_HOST"), os.Getenv("MYSQL_PORT"), dbName)
